...
```bash
#一般模式三哨兵两主从, 哨兵负责监控主从服务器, 一旦发现故障则会进行切换
#效果一样
...
* 主从模式是实现读写分离
* 单机模式单应用可用
* 集群模式实现更高并发量、更可靠，单机模式已经无法满足情况,通过工具redis-trib实现重新分片
![redis集群分片过程](../images/2019/05/redis集群分片过程.png)
![redis集群重新分配分片](../images/2019/05/redis集群重新分配槽点.png)
...
* 一个Redis实例最多能存放多少的keys？List、Set、Sorted Set他们最多能存放多少元素
    * 理论上Redis可以处理多达232的keys，并且在实际中进行了测试，每个实例至少存放了2亿5千万的keys
* Redis常见性能问题和解决方案？
